...
<script>
    export default {
        ...
        methods: {
            initNoticeData() {
                this.loading = true;
                this.getRequest('/inotice/info/').then(resp => {
                    this.loading = false;
                    if (resp) {
            ...
            //真正的更新
            updateNotice() {
                this.putRequest('/inotice/info/', this.editNoticeData).then(resp => {
                    if (resp) {
                        this.initNoticeData();
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除通知, 是否继续?', '提示', {
                    ...
                    this.deleteRequest('/inotice/info/' + data.id).then(resp => {
                        if (resp) {
                            this.initNoticeData();
                        }
                    })
            ...
            addNotice(){
                if (this.addNoticeData.title && this.addNoticeData.content) {
                    this.postRequest('/inotice/info/', this.addNoticeData).then(resp => {
                        if (resp) {
                            this.initNoticeData();
